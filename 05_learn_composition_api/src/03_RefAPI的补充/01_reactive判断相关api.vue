...
export default {
  setup() {
    ...
    // reactive判断的相关api
    // 1.isProxy--是否是reactive,readonly、shallowReactive、shallowReadonly创建的代理对象
    console.log(isProxy(info))
    console.log(isProxy(readonlyInfo))

    // 2.isReactive--是否是reactive\shallowReactive创建出来的响应式代理对象
    console.log(isReactive(info))
    console.log(isReactive(readonlyInfo))

    // 3.isReadonly--是否是readonly、shallowReadonly创建出来的代理对象
    console.log(isReadonly(readonlyInfo))

    // 4.toRaw--获取reactive和readonly、shallowReactive、shallowReadonly创建的代理对象的原来的对象
